...
def zrobOknoEdycjiOcen(wartosci, siatkaDanychOcen):
    ...
    labelPrzedmiot = Label(noweOkno, text="Przedmiot: ", width=15).grid(row=0, column=0)
    przedmiotCombo = ttk.Combobox(noweOkno, values=['matematyka', 'informatyka', 'fizyka', 'geografia'], width=30)
    przedmiotCombo.set(wartosci[1])
    przedmiotCombo.grid(row=0, column=1)
    ...
    def zrob():
        dane = (wartosci[0], przedmiotCombo.get(), ocenaEntry.get(), wartosci[3])
